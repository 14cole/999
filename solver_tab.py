...
class SolverTab(QWidget):
    ...
    def _build_left_panel(self) -> QWidget:
        ...
        self.cmb_display_norm = QComboBox()
        self.cmb_display_norm.addItem("2D width: σ = (numerator / k) |A|²", userData="divide_by_k")
        self.cmb_display_norm.addItem("2D width: σ = numerator · |A|²", userData="constant")
        self.cmb_display_norm.addItem("Raw amplitude power: |A|²", userData="off")
        self.cmb_display_norm.setCurrentIndex(0)
        self.edit_display_numerator = QLineEdit("0.25")
        self.edit_display_numerator.setToolTip("Used when the display mode applies a numerator.")
        ...
        options_form.addRow("Display Quantity", self.cmb_display_norm)
        options_form.addRow("Display Numerator", self.edit_display_numerator)
        ...
        self.cmb_display_norm.currentIndexChanged.connect(self._update_display_mode_controls)
        self.cmb_display_norm.currentIndexChanged.connect(self._refresh_result_views)
        self.edit_display_numerator.editingFinished.connect(self._refresh_result_views)
    ...
    def _build_right_panel(self) -> QWidget:
        ...
        self.table_results.setHorizontalHeaderLabels(
            ["Frequency (GHz)", "Elevation (deg)", "2D Width (linear)", "2D Width (dB)"]
        )
    ...
    def _update_display_mode_controls(self):
        mode = str(self.cmb_display_norm.currentData() or "divide_by_k")
        self.edit_display_numerator.setEnabled(mode != "off" and not self._is_solving)

    def _display_numerator_value(self) -> float:
        text = self.edit_display_numerator.text().strip()
        if not text:
            return 0.25
        value = float(text)
        if (not math.isfinite(value)) or value < 0.0:
            raise ValueError("Display numerator must be a finite value >= 0.")
        return value

    def _current_display_config(self) -> Dict[str, Any]:
        mode = str(self.cmb_display_norm.currentData() or "divide_by_k")
        numerator = self._display_numerator_value() if mode != "off" else None
        return {"mode": mode, "numerator": numerator}

    def _display_formula_text(self, mode: str, numerator: float | None) -> str:
        if mode == "off":
            return "|A|²"
        if mode == "constant":
            return f"σ₂D = ({float(numerator):.6g}) · |A|²"
        return f"σ₂D = ({float(numerator):.6g}/k) · |A|²"

    def _display_headers(self) -> Tuple[str, str, str]:
        mode = str(self.cmb_display_norm.currentData() or "divide_by_k")
        numerator = self._display_numerator_value() if mode != "off" else None
        if mode == "off":
            return ("Raw |A|²", "Raw |A|² (dB)", "Monostatic 2D Raw Amplitude Power")
        if mode == "constant":
            label = f"2D Width, σ = ({float(numerator):.6g})|A|²"
            return (f"{label} (linear)", f"{label} (dB)", "Monostatic 2D Scattering Width")
        label = f"2D Width, σ = ({float(numerator):.6g}/k)|A|²"
        return (f"{label} (linear)", f"{label} (dB)", "Monostatic 2D Scattering Width")

    def _display_linear_value(self, row: Dict[str, Any]) -> float:
        mode = str(self.cmb_display_norm.currentData() or "divide_by_k")
        amp = complex(float(row.get("rcs_amp_real", 0.0) or 0.0), float(row.get("rcs_amp_imag", 0.0) or 0.0))
        amp_power = float(abs(amp) ** 2)
        if amp_power <= 0.0:
            amp_power = max(float(row.get("rcs_linear", 0.0) or 0.0), 0.0)
        if mode == "off":
            return amp_power
        numerator = self._display_numerator_value()
        if mode == "constant":
            return float(numerator) * amp_power
        freq_ghz = float(row.get("frequency_ghz", 0.0) or 0.0)
        k_value = 2.0 * math.pi * (freq_ghz * 1.0e9) / C0
        return (float(numerator) / max(k_value, 1.0e-12)) * amp_power

    def _refresh_result_views(self):
        self._update_display_mode_controls()
        if not self.last_result:
            lin_header, db_header, _ = self._display_headers()
            self.table_results.setHorizontalHeaderLabels(["Frequency (GHz)", "Elevation (deg)", lin_header, db_header])
            return
        try:
            self._populate_results_table(self.last_result)
            self._plot_results(self.last_result)
        except Exception as exc:
            QMessageBox.warning(self, "Display Mode", f"Could not refresh displayed normalization:\n{exc}")
    ...
    def _set_solving_state(self, solving: bool):
        ...
        self.cmb_display_norm.setEnabled(not solving)
    ...
    def _on_solver_finished(self, result: Dict[str, Any], source_path: str):
        ...
        display_formula = self._display_formula_text(display_cfg["mode"], display_cfg.get("numerator"))
        self.lbl_status.setText(write_summary + quality_suffix + mesh_suffix + f" Display: {display_formula}.")
    ...
    def _run_solver(self):
        ...
        try:
            ...
            _ = self._current_display_config()
    ...
    def _populate_results_table(self, result: Dict[str, Any]):
        ...
        for r, row in enumerate(rows):
            ...
            lin = float(self._display_linear_value(row))
            db = float(10.0 * math.log10(max(lin, 1e-12)))
    ...
    def _plot_results(self, result: Dict[str, Any]):
        ...
        _, db_header, plot_title = self._display_headers()
        ax = self.canvas.ax
        ax.clear()
        for freq in sorted(by_freq.keys()):
            ...
            y = [float(10.0 * math.log10(max(self._display_linear_value(row), 1e-12))) for row in rows]
            ax.plot(x, y, linewidth=1.8, label=f"{freq:g} GHz")

        ax.set_title(plot_title)
        ax.set_xlabel("Elevation (deg)")
        ax.set_ylabel(db_header)
